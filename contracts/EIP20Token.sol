...
contract EIP20Token is EIP20Interface {
    ...
    */
    ...
    function transfer(address _to, uint256 _value) public returns (bool success) {
        bytes memory vide;
        return transfer(_to, _value, vide);
    }

    function transfer(address _to, uint _value, bytes _data) public returns (bool){
        if(isContract(_to)){
            ContractReceiver rx = ContractReceiver(_to);
            bytes4 _support = bytes4(keccak256('tokenFallback(_to, _value,data)'));
            require(rx.supportsInterface(_support));
            rx.tokenFallback( msg.sender, _value, _data);
        }
